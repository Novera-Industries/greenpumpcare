    ...
    <>
      ...
      <AnimatedSection id="services" className="section scroll-mt-24">
        <div className="container">
          ...
          <StaggerContainer className="hidden md:grid grid-cols-3 gap-6 mb-8">
      ...
      <AnimatedSection id="repairs" className="section scroll-mt-24">
        <div className="container">
          ...
          {/* Desktop: stagger grid */}
          <StaggerContainer className="hidden md:grid grid-cols-3 gap-6 mb-10 max-w-5xl mx-auto">
            <StaggerItem hover>
              <RepairCard icon={Stethoscope} label="Diagnostic Visit" price="$99" body="Flat-rate diagnostic fee, applied toward the repair cost if you proceed." />
            </StaggerItem>
            <StaggerItem hover>
              <RepairCard icon={Wrench} label="Repair Labour" price="$125" suffix="/hr" body="Transparent hourly rate. Quality parts billed separately, always quoted before work begins." />
            </StaggerItem>
            <StaggerItem hover>
              <RepairCard icon={Percent} label="Care Plan Savings" price="Up to 20%" body="All repairs discounted for Care Plan members. 10%, 15%, or 20% based on your tier." />
            </StaggerItem>
          </StaggerContainer>

          {/* Mobile: forward marquee */}
          <MobileMarquee speed={46} className="mb-10" itemClassName="w-[260px] shrink-0 flex">
            <RepairCard icon={Stethoscope} label="Diagnostic Visit" price="$99" body="Flat-rate diagnostic fee, applied toward the repair cost if you proceed." />
            <RepairCard icon={Wrench} label="Repair Labour" price="$125" suffix="/hr" body="Transparent hourly rate. Quality parts billed separately, always quoted before work begins." />
            <RepairCard icon={Percent} label="Care Plan Savings" price="Up to 20%" body="All repairs discounted for Care Plan members. 10%, 15%, or 20% based on your tier." />
          </MobileMarquee>
...
function PricingServiceCard({ service }: { service: (typeof SERVICES)[number] }) {
...
function PricingBundleCard({ bundle }: { bundle: (typeof BUNDLES)[number] }) {
  return (
    <div className="bg-stripe rounded-card shadow-card p-6 flex flex-col h-full w-full text-center md:text-left">
      <Package className="w-8 h-8 text-primary mb-3 mx-auto md:mx-0" />
      <h3 className="font-heading text-lg font-semibold text-text mb-1.5">
        {bundle.name}
      </h3>
      <p className="text-gray-600 text-sm flex-1 mb-4">{bundle.description}</p>
      <div className="flex flex-col items-center md:flex-row md:items-end md:justify-between pt-3 border-t border-white/60">
        <div>
...
function RepairCard({
  ...
  return (
    <div className="bg-white rounded-card shadow-card p-8 h-full w-full flex flex-col text-center">
      <Icon className="w-10 h-10 text-primary mx-auto mb-4" />
      ...
      <p className="mb-2">
        ...
        {suffix && (
          <span className="text-gray-500 text-lg font-medium">{suffix}</span>
        )}
      </p>
