...
function PlanCard({
  ...
  plan: (typeof CARE_PLANS)[number];
  animated?: boolean;
}) {
  return (
    <div
      className={cn(
        "relative rounded-card p-8 h-full flex flex-col overflow-hidden card-premium w-full text-center md:text-left",
        plan.isPopular
    ...
    >
      ...
      {plan.isPopular && (
        <span className="relative z-10 inline-flex items-center gap-2 self-center md:self-start bg-primary/30 text-mint text-xs font-semibold px-3 py-1.5 rounded-pill mb-4">
        ...
        {plan.features.map((feature, i) =>
          animated ? (
            <motion.li
              ...
              className={cn(
                "flex items-start justify-center md:justify-start text-center md:text-left gap-2 text-sm whitespace-pre-line",
